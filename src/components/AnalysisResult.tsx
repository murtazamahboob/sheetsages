...
const AnalysisResult = ({ data, fileName, onNewAnalysis, userPlan = "free" }: AnalysisResultProps) => {
  ...
  const handleDownloadPDF = () => {
    ...
    // Summary
    addWrappedText("📌 Summary", 16, true);
    ...
    if (data.insights.length > 0) {
      addWrappedText("🔍 Key Insights", 16, true);
    ...
    if (data.warnings.length > 0) {
      addWrappedText("⚠️ Warnings / Risks", 16, true);
    ...
    if (data.opportunities.length > 0) {
      addWrappedText("🚀 Opportunities", 16, true);
    ...
    if (data.actions.length > 0) {
      addWrappedText("✅ Recommended Actions", 16, true);
